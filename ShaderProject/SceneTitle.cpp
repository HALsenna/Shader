...
const char* szUIFileName[TITLEUI_MAX]
{
	...
	"Assets/Texture/Title/TEX_Title_02.png",		// タイトルロゴ(名前)
...
void SceneTitle::Init()
{
	...
	obj = { "flowerpetalModel", flowerpetalFile, XMFLOAT3(0.0f, 0.0f, 0.0f),
		XMFLOAT3(0.0f, 0.0f, 0.0f), 0.1f, VS_OBJECT, PS_FLOWER };
	SetObjInfo(obj);

	obj = { "flowerstemModel", flowerstemFile, XMFLOAT3(0.0f, 0.0f, 0.0f),
		XMFLOAT3(0.0f, 0.0f, 0.0f), 0.1f, VS_OBJECT, PS_OBJECT };
	SetObjInfo(obj);
	...
	for(int i = 0; i < TITLEUI_MAX; ++i)
	{
		g_pObjUI[i] = cui[i];
		UIINFO uiInitData;
		uiInitData.color = UIColor[i];
		uiInitData.kind = UI_NONE;
		uiInitData.pos = TitleUIPos[i];
		uiInitData.pTex = g_pTex[i];
		uiInitData.size = TitleUISize[i];
		uiInitData.ps = PS_2D;
		uiInitData.vs = VS_2D;
		g_pObjUI[i]->Init(uiInitData);
	}
}
CUI** CreateUI()
{
...
void SceneTitle::Draw()
{
	...
	//	XMFLOAT4{color.x, color.y, color.z, 1.0f},
	//};
