...
void SceneBaseShader::Init()
{
	...
	m_isView[SHADER_posECTIONLIGHT] = true;
...
{
	// ライトの方向の設定
	m_posectionLight.lightDir = { 1.0f, 1.0f, -1.0f, 1.0f };
	// ライトのカラーの設定
	m_posectionLight.lightColor = { 1.0f, 0.0f, 0.0f, 1.0f };
}
#pragma endregion 
...
{
	bOldImgFlg = m_ImGUIFlg[CB_posTIONLIGHT];
	// スイッチ
	ImGui::Checkbox("DIRECTIONLIGHT", &m_ImGUIFlg[CB_posTIONLIGHT]);
	if (m_ImGUIFlg[CB_posTIONLIGHT] != bOldImgFlg)
	{
		// ディレクションライトを付ける
		m_isView[SHADER_posECTIONLIGHT] = !m_isView[SHADER_posECTIONLIGHT];
	}

	// 数値入力
	static float sliderPos[3] = { 1.0f, -1.0f, 1.0f };
	ImGui::SliderFloat3("pos", sliderPos, -1.0f, 1.0f);
	m_posectionLight.lightDir = { sliderPos[0], -sliderPos[1], sliderPos[2], 0.0f};
	static float sliderColor[3] = { 0.0f, 0.0f, 0.0f };
	ImGui::SliderFloat3("color", sliderColor, 0.0f, 1.0f);
	m_posectionLight.lightColor = { sliderColor[0], sliderColor[1], sliderColor[2], 1.0f };
}
#pragma endregion 
...
{
	_pLight->Write(&m_posectionLight);
	_pLight->BindPS(0);
}
#pragma endregion 
