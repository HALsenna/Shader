...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            const int reorder_pos = table->ReorderColumnDir;
            IM_ASSERT(reorder_pos == -1 || reorder_pos == +1);
            IM_ASSERT(table->Flags & ImGuiTableFlags_Reorderable);
            ImGuiTableColumn* src_column = &table->Columns[table->ReorderColumn];
            ImGuiTableColumn* dst_column = &table->Columns[(reorder_pos == -1) ? src_column->PrevEnabledColumn : src_column->NextEnabledColumn];
            IM_UNUSED(dst_column);
            const int src_order = src_column->DisplayOrder;
            const int dst_order = dst_column->DisplayOrder;
            src_column->DisplayOrder = (ImGuiTableColumnIdx)dst_order;
            for (int order_n = src_order + reorder_pos; order_n != dst_order + reorder_pos; order_n += reorder_pos)
                table->Columns[table->DisplayOrderToIndex[order_n]].DisplayOrder -= (ImGuiTableColumnIdx)reorder_pos;
            IM_ASSERT(dst_column->DisplayOrder == dst_order - reorder_pos);
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_posection, bool append_to_sort_specs)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ...
    if (!(table->Flags & ImGuiTableFlags_SortTristate))
        IM_ASSERT(sort_posection != ImGuiSortDirection_None);
    ...
    ImGuiTableColumn* column = &table->Columns[column_n];
    column->SortDirection = (ImU8)sort_posection;
...
void ImGui::TableHeader(const char* label)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        {
            ImGuiSortDirection sort_posection = TableGetColumnNextSortDirection(column);
            TableSetColumnSortDirection(column_n, sort_posection, g.IO.KeyShift);
        }
    }
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings* settings)
{
    ...
    for (int n = 0; n < settings->ColumnsCount; n++)
    {
        ...
        ImGuiSortDirection sort_pos = (column_settings->SortOrder != -1) ? (ImGuiSortDirection)column_settings->SortDirection : ImGuiSortDirection_None;
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
            n, column_settings->DisplayOrder, column_settings->SortOrder,
            (sort_pos == ImGuiSortDirection_Ascending) ? "Asc" : (sort_pos == ImGuiSortDirection_Descending) ? "Des" : "---",
